...
export default function TeacherNotices() {
  ...
  const [form, setForm] = useState({
    title: "", content: "", targetAudience: "Students" as Notice["targetAudience"],
  ...
  const resetForm = () => setForm({ title: "", content: "", targetAudience: "Students", targetClasses: [] });
      ...
      <div className="space-y-1">
        ...
        {form.targetClasses.length > 0 && (
          <p className="text-xs text-muted-foreground">
            Targeting: {form.targetClasses.join(", ")}
          </p>
        )}
      </div>
      <div className="space-y-1">
        <Label>Content</Label>
        <Textarea rows={5} placeholder="Write notice content..." value={form.content} onChange={e => setForm({ ...form, content: e.target.value })} />
      </div>
    </div>
  );

  const NoticeCard = ({ n, editable }: { n: Notice; editable?: boolean }) => (
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1">
            <div className="flex flex-wrap items-center gap-2 mb-1">
              ...
              {n.authorRole === "principal" && <Badge className="text-xs bg-primary">Principal</Badge>}
      ...
      <Tabs defaultValue="mine">
        ...
        <TabsContent value="mine" className="mt-4 space-y-4">
          {myNotices.length === 0 ? (
            ...
            myNotices.map(n => <NoticeCard key={n.id} n={n} editable />)
          )}
        </TabsContent>
        ...
        <TabsContent value="school" className="mt-4 space-y-4">
          {schoolNotices.length === 0 ? (
            ...
            schoolNotices.map(n => <NoticeCard key={n.id} n={n} />)
          )}
        </TabsContent>
      </Tabs>
